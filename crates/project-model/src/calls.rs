...
use crate::calls_text_fallback::extract_calls_text_fallback;
use crate::model::*;
...
pub fn extract_and_resolve_calls(
    ...
    symbol_index.set_wildcard_modules(build_wildcard_module_map(imports));
...
pub fn extract_delta_calls(
    ...
    let import_bindings = ImportBindingTable::empty();
...
pub(crate) fn resolve_free_function(
    call: &mut CallSite,
    symbol_index: &CalleeIndex,
    import_bindings: &ImportBindingTable,
) {
    ...
    match same_crate.as_slice() {
        ...
        multiple if !multiple.is_empty() => {
            if let Some(wildcard_imported) = symbol_index.wildcard_modules_for(&call.source_path) {
...
pub(crate) fn resolve_associated_function(
    call: &mut CallSite,
    symbol_index: &CalleeIndex,
    import_bindings: &ImportBindingTable,
) {
