...
use crate::extractors::include::{CppInclude, CppIncludeKind};
...
pub enum CppResolvedIncludeKind {
...
pub struct CppResolvedInclude {
    ...
    pub resolution_kind: CppResolvedIncludeKind,
...
impl CppIncludeResolver {
    ...
    pub fn resolve(&self, source_file: &Path, include: &CppInclude) -> CppResolvedInclude {
        match include.kind {
            CppIncludeKind::Local => self.resolve_local(source_file, &include.path),
            CppIncludeKind::System => self.resolve_system(source_file, &include.path),
        }
    }

    fn resolve_local(&self, source_file: &Path, include_path: &str) -> CppResolvedInclude {
        let entry = self
            .compile_db
            .as_ref()
        ...
        if let Some(parent) = source_file.parent() {
            ...
            if candidate.is_file() {
                return CppResolvedInclude {
                    include_path: include_path.to_string(),
                    target_file: Some(candidate),
                    resolution_kind: CppResolvedIncludeKind::SameDirectory,
                    confidence: Some(CONFIDENCE_SAME_DIR),
                    reason: "cpp-local-include-same-directory".to_string(),
                };
            }
        }
            ...
            for dir in &e.quote_include_dirs {
                ...
                if candidate.is_file() {
                    return CppResolvedInclude {
                        include_path: include_path.to_string(),
                        target_file: Some(candidate),
                        resolution_kind: CppResolvedIncludeKind::QuoteIncludeDir,
                        confidence: Some(CONFIDENCE_QUOTE_DIR),
                        reason: "cpp-quote-include-dir".to_string(),
                    };
                }
            }
        }
            ...
            for dir in &e.project_include_dirs {
                ...
                if candidate.is_file() {
                    return CppResolvedInclude {
                        include_path: include_path.to_string(),
                        target_file: Some(candidate),
                        resolution_kind: CppResolvedIncludeKind::ProjectIncludeDir,
                        confidence: Some(CONFIDENCE_PROJECT_DIR),
                        reason: "cpp-project-include-dir".to_string(),
                    };
                }
            }
        }
        ...
        if let Some(candidates) = self.basename_index.get(basename) {
            if candidates.len() == 1 {
                return CppResolvedInclude {
                    ...
                    reason: "cpp-filename-unique-fallback".to_string(),
                };
            }
            return CppResolvedInclude {
                include_path: include_path.to_string(),
                target_file: None,
                resolution_kind: CppResolvedIncludeKind::Ambiguous,
                confidence: None,
                reason: "cpp-include-ambiguous".to_string(),
            };
        }
        ...
        CppResolvedInclude {
            include_path: include_path.to_string(),
            target_file: None,
            resolution_kind: CppResolvedIncludeKind::Unresolved,
            confidence: None,
            reason: "cpp-include-unresolved".to_string(),
        }
    }

    fn resolve_system(&self, source_file: &Path, include_path: &str) -> CppResolvedInclude {
        let entry = self
            .compile_db
            .as_ref()
            .and_then(|db| db.for_file(source_file));
                ...
                if candidate.is_file() {
                    if candidate.starts_with(&self.project_root) {
                        return CppResolvedInclude {
                            ...
                            reason: "cpp-project-include-angle-resolved".to_string(),
                        };
                    }
                }
            }
        ...
        CppResolvedInclude {
            include_path: include_path.to_string(),
            target_file: None,
            resolution_kind: CppResolvedIncludeKind::SystemExternal,
            confidence: None,
            reason: "cpp-system-include-external".to_string(),
        }
    }

    /// Resolve forced includes for a source file.
    pub fn resolve_forced_includes(&self, source_file: &Path) -> Vec<CppResolvedInclude> {
        ...
        for forced_path in &entry.forced_includes {
            if forced_path.is_file() && forced_path.starts_with(&self.project_root) {
                results.push(CppResolvedInclude {
                    include_path: forced_path.to_string_lossy().to_string(),
                    target_file: Some(forced_path.clone()),
                    resolution_kind: CppResolvedIncludeKind::ForcedInclude,
                    confidence: Some(CONFIDENCE_FORCED),
                    reason: "cpp-forced-include".to_string(),
                });
            }
        }
        results
