...
/// Parse compiler flags and populate the CompileCommandEntry.
...
/// Parse a -D define into (name, optional value).
...
/// Resolve a directory path relative to the compile directory or project root.
    ...
    if path.is_absolute() {
        ...
        if from_dir.exists() {
            ...
            if from_root.exists() {
                ...
                // Return directory-relative path even if it doesn't exist yet
                Some(from_dir)
            }
        }
    }
...
/// Resolve a file path relative to the compile directory or project root.
...
mod tests {
    ...
    fn test_shell_split_simple() {
        let result = shell_split("cc -Iinclude -c src/main.c");
        assert_eq!(result, vec!["cc", "-Iinclude", "-c", "src/main.c"]);
    }

    #[test]
    fn test_shell_split_quotes() {
        let result = shell_split("cc -DNAME=\"hello world\" -c main.c");
        assert_eq!(result, vec!["cc", "-DNAME=hello world", "-c", "main.c"]);
    }

    #[test]
    fn test_shell_split_single_quotes() {
        let result = shell_split("cc -DVAL='foo bar' -c main.c");
        assert_eq!(result, vec!["cc", "-DVAL=foo bar", "-c", "main.c"]);
    }

    #[test]
    fn test_parse_define_simple() {
        assert_eq!(parse_define("APP_DEBUG"), ("APP_DEBUG".into(), None));
    }

    #[test]
    fn test_parse_define_with_value() {
        assert_eq!(
            parse_define("APP_DEBUG=1"),
            ("APP_DEBUG".into(), Some("1".into()))
        );
    }
}
