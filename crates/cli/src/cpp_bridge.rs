...
//! 处理：
//! 1. Node 分类：repository / source-file / header-file / namespace / symbol
...
pub fn convert_cpp_graph(
    ...
    // 从 nodes 按 kind 分类提取
    let (repo_node, source_files, symbols) = partition_cpp_nodes(nodes)?;
    ...
    let bridge_edges = convert_cpp_edges(edges);
...
fn partition_cpp_nodes(
    ...
    let repo = repo_node.ok_or("C++ graph JSON 缺少 repository node")?;
    Ok((repo, source_files, symbols))
}

/// 将 C++ edges 转换为 Bridge 格式并按 kind 分组
fn convert_cpp_edges(edges: &[Value]) -> BridgeEdges {
    let bridge_edges: Vec<BridgeEdge> = edges
        ...
        .map(|edge| {
            ...
            // Extract confidence/reason from properties
