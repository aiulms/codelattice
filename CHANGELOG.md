...
- **JS/TS Real-World Corpus Validation Pack**: adds corpus-level validation infrastructure for JavaScript/TypeScript analysis. Includes `fixtures/corpus/js-ts-corpus-manifest.json` with 6 candidate projects (lodash-es, uuid, zod, tinybench, vite-plugin-react, conf) and rewritten `scripts/js-ts-corpus-smoke.sh` supporting `--fixture-only`, `--corpus-dir`, `--clone-missing`, `--offline` modes. Script writes structured JSON report to `/tmp/codelattice-js-ts-corpus-report.json`. Fixture-only baseline: JS portable-smoke (11 files, 51 symbols, 104 edges, 11 CALLS), TS portable-smoke (4 files, 20 symbols, 54 edges, 12 CALLS). GitHub clone gracefully skipped when network unavailable.
