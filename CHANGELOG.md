...
- **MCP memory cache P0**: GraphView nodes/edges now use shared `Arc<Value>` allocations across primary/secondary indexes, and `CacheEntry` shares one GraphView container plus one raw analyze JSON across repeated MCP queries. Persistent cache schema v3 stores only analyze JSON (dropping the larger duplicate typed-graph snapshot), writes through a borrowed buffered serializer, and rejects legacy schema from a 4 KiB prefix without materializing its graph. Cache status reads metadata with `IgnoredAny`, while project-once engine artifacts are persistent-only and loaded on demand. Controlled release benchmark on CodeLattice self-analysis: fresh warm peak 108.1 MiB / steady 102.3 MiB; cross-process persistent warm peak 74.6 MiB; three repeated queries stayed flat. A synthetic 165 MiB legacy cache status scan added only 0.4 MiB RSS (previous field report: 1.3 GiB peak when full entries were deserialized).
...
- **Ask orchestration + whatif previews**: `codelattice_workflow(mode=ask)` now auto-orchestrates multi-step analysis for `explain_flow` (call_chains + readOrder), `locate_issue` (project diagnose + triagePlan), `inspect_project`, and `before_edit` (whatif preview). New `codelattice_change_review(mode=whatif)` computes pre-edit impact previews with `targetCandidates`, `directImpact`, `indirectImpact`, `risk` level, `safeAlternatives`, and `testsToRun` — all static-only. Schema: `codelattice.whatIf.v1`. Ask v2 schema preserved with additive `triagePlan` and `whatIf` fields. 6 new MCP regression tests (total 172). Dev-only, installed CodeLattice-Tool not synced.
...
- **Architecture Insight Quality Pack**: improves `codelattice_project_insights` for AI onboarding and problem localization. Entry point candidates are now classified (`runtime_start`, `framework_route`, `cli`, `package_entry`, `public_api`, `orchestration`, `test_entry`) with confidence, evidence, and next actions; test-like entries are downranked so they do not dominate architecture entry recommendations. Adds dimensioned `architectureRisk` output for entry point quality, graph density, low-confidence edges, diagnostics, and hotspot complexity instead of a flat always-LOW interpretation. Workspace auto-entry and `rootDiagnosis` now explain `sourceOnly` entries with category, reason, and next action.
...
- **Native Change Intelligence Pack**: upgrades `detect-changes` CLI and MCP facade with workspace-aware change intelligence. Adds file-to-project ownership mapping, cross-project impact propagation, three-layer risk grading (`max(assist_risk, symbol_risk, workspace_risk)`), enhanced risk reasons, and recommended followups. New output fields: `workspaceContext`, `fileOwners`, `affectedProjects`, `affectedWorkspaceEdges`, `unsupportedBoundaryHits`, `crossProjectRisk`, `recommendedFollowups`. Precommit script upgraded with Chinese workspace summary and JSON report output. `codelattice_change_review` facade gains `native_review` mode. Detect-changes smoke: 17/17 pass (4 new workspace checks).

- **JS/TS Real-World Corpus Validation Pack**: adds corpus-level validation infrastructure for JavaScript/TypeScript analysis. Includes `fixtures/corpus/js-ts-corpus-manifest.json` with 6 candidate projects (lodash, uuid, zod, tinybench, vite-plugin-react, conf) using GitCode mirrors and rewritten `scripts/js-ts-corpus-smoke.sh` supporting `--fixture-only`, `--corpus-dir`, `--clone-missing`, `--offline` modes. Script writes structured JSON report to `/tmp/codelattice-js-ts-corpus-report.json`. Real-world baselines use manifest-pinned languages: lodash (10,775 symbols, 3,592 CALLS), uuid as JavaScript (86 symbols, 21 CALLS), zod (2,845 symbols, 34,636 CALLS), tinybench (97 symbols, 3,101 CALLS), vite-plugin-react as TypeScript (849 symbols, 6,534 CALLS), conf (68 symbols, 1,612 CALLS). All 8/8 projects pass (6 real + 2 fixtures).

- **JavaScript Phase A Support**: adds JavaScript as a formally supported language. Supports `.js`, `.jsx`, `.mjs`, `.cjs` file analysis using tree-sitter-typescript grammar as parser backend. Includes ESM `import`/`export`, CommonJS `require`/`module.exports`/`exports.foo`, function/class/method/arrow function/component symbol extraction, relative path module resolution, `package.json` entry detection (`main`/`module`/`exports`/`bin`), and dynamic `import()` diagnostics. CLI `--language javascript`, MCP `language=javascript`, and WebUI language selector all support JavaScript. JavaScript mode only analyzes JS/JSX files; TypeScript mode is not affected. Parser backend reuses `gitnexus-typescript`'s tree-sitter-typescript (TypeScript grammar is backward-compatible with JavaScript). `crates/javascript/` is a dedicated JavaScript adapter with clear boundary from TypeScript adapter.

- **JS/TS Real-World Hardening**: enhances JavaScript/TypeScript analysis for real-world project reliability. Adds `package.json` `browser`/`types`/`typings` field parsing, conditional `exports` resolution (`import`/`require`/`default`/`node`/`browser`), `bin` string-or-object normalization, and `entry_points` structured list. Adds framework detection heuristics (React/Next.js/Express/Koa/Fastify/Vite/webpack) via `dependencies`/`devDependencies`/`scripts` analysis. Adds public API surface extraction with `externalUsageVerified=false` and `staticOnly=true` markers. Adds barrel export detection (`index.js` files with re-exports). Adds `summary` JSON output with structured baseline counts (source files, symbols, edges, diagnostics, dynamic imports, unresolved imports, external imports, framework hints, public surface candidates, entry points). Adds `scripts/js-ts-corpus-smoke.sh` for fixture + optional real-project read-only smoke testing. Enhances `fixtures/javascript/portable-smoke` with Express route handlers, nested barrel exports, bin entry, and richer `package.json`. All heuristic results marked `runtimeVerified=false`.
...
- **MCP stable tool visibility**: removes duplicate `codelattice_workspace_graph` / `codelattice_cross_project_impact` definitions from full `tools/list`, adds a uniqueness assertion to the MCP test suite, updates full-toolset checks to the 49 unique-tool profile, and promotes the cleaned runtime into `/Users/jiangxuanyang/Desktop/CodeLattice-Tool`. Default client mode remains the smaller `ai` toolset.
...
- **Incremental Dirty-file Planning Pack**: extends the scheduler with a compact file snapshot and `incrementalPlan` metadata for stale cache paths. Memory and persistent cache entries now preserve scheduler-tracked file snapshots, so a cache miss can name changed files, summarize added/modified/removed counts, list affected phases, and label source-only changes as `fileScopedCandidate` for future partial graph rebuild work. This is planning metadata only; MCP still runs the existing full analysis path on cache miss.

- **AI Workflow Executor Pack**: extends `codelattice_workflow` with `execute=true` so AI agents can run the recommended non-recursive facade actions in one call. The response now includes `execution`, `completedActions`, `failedActions`, `skippedActions`, compact `evidence`, and `answerSummary`. Missing required inputs stop execution with `execution.status=needs_input` and preserve discovery `nextActions`, preventing agents from blindly analyzing the wrong target. Also hardens `codelattice_symbol` facade callers/callees mode so `name` is accepted as a `symbol` alias.

- **AI Tool Experience Pack**: upgrades the default `codelattice_workflow` facade from a preset wrapper into an AI intent router. It now returns an `ai.workflow.v1` action envelope with `situation`, `riskLevel`, `confidence`, `findings`, `missingInputs`, directly callable `nextActions`, static-only `cautions`, `humanReviewNeeded`, and `safeToProceed`. Supported intents include onboarding, before_edit, after_edit, delete_code, release_check, legacy_cleanup, workspace_review, cross_project_impact, explain_symbol, docs_tests_sync, config_examples_sync, public_api_change, and framework_route_change. Missing inputs no longer produce dead ends: before_edit without a symbol routes to `codelattice_symbol mode=search`; cross_project_impact without a target routes to `codelattice_workspace mode=graph`.

- **MCP AI Toolset Default Pack**: changes the default MCP surface from full 49-tool exposure to an AI-friendly `ai` toolset. The current default `tools/list` returns 6 entry tools (`codelattice_workflow`, `codelattice_project`, `codelattice_symbol`, `codelattice_change_review`, `codelattice_workspace`, `codelattice_cache`). `CODELATTICE_MCP_TOOLSET=core` exposes common low-level tools for advanced agents, and `CODELATTICE_MCP_TOOLSET=full` keeps all 49 tools for debugging and regression smoke. `initialize.serverInfo` now includes `toolset`, visible `toolCount`, `fullToolCount`, and `recommendedEntryTools`. Hidden tools return `tool_not_in_ai_toolset` / `tool_not_in_core_toolset` with facade and env-var guidance instead of letting agents guess.

- **Workspace Impact Precision Pack**: reduces noise in CodeLattice-native `detect-changes` workspace impact output for large repos with fixtures and smoke corpora. A new precision layer classifies affected project surfaces (production, fixture, test, docs, webui, script, unsupported), groups impacts into direct/high-confidence/low-confidence/fixture-only/unsupported buckets, and defaults to daily mode that reports direct + high-confidence impacts while summarizing fixture/test/adjacency-only noise in `suppressedProjects` and `workspaceImpactSummary`. New CLI flags: `--include-fixtures` expands fixture/test/demo impacts for audit runs, and `--strict-workspace` preserves all workspace graph impacts. `recommendedFollowups` now uses capped grouped summaries instead of dumping every downstream project label.

- **Native Change Intelligence Pack**: upgrades `codelattice detect-changes` from single-project change review to workspace-aware change intelligence. New output fields: `workspaceContext` (isWorkspace, projectCount, workspaceGraphAvailable), `fileOwners` (per-file project ownership mapping with ownerKind: project/config/script/unsupported/unknown), `affectedProjects`, `affectedWorkspaceEdges`, `unsupportedBoundaryHits`, `crossProjectRisk`, `recommendedFollowups`, enhanced `riskReasons` (workspace-aware). Risk grading now uses three-layer stacking: max(production_assist_risk, changed_symbol_risk, workspace_risk). Precommit script updated with Chinese-friendly workspace summary and `/tmp/codelattice-change-review.json` output. `codelattice_change_review` facade gains `native_review` mode. New workspace fixture files in `fixtures/workspace/multi-project/`. All new fields are additive — existing v1 fields unchanged, no schema version bump. Workspace graph failures graceful degrade to empty arrays with reason.

- **CodeLattice Native Governance Migration**: adds `scripts/codelattice-precommit-check.sh` as the first-party daily precommit bundle. The script runs formatting, whitespace diff checks, productization regression, MCP regression, native detect-changes smoke, and a final `codelattice detect-changes` summary without calling GitNexus-Tool by default. `AGENTS.md`, README, and MCP local setup docs now prefer CodeLattice-native governance; legacy GitNexus-Tool is documented as fallback/comparison only.

- **CodeLattice-native detect-changes CLI**: adds `codelattice detect-changes` as a first-party pre-commit change review entry point. The command reuses local MCP `codelattice_changed_symbols` and `codelattice_production_assist` under the hood, maps `--scope all` to `git diff HEAD`, additionally reports untracked files via `git ls-files --others --exclude-standard`, and emits `codelattice.detectChanges.v1` JSON with changed files, changed symbols, unknown hunks, risk summary, review checklist, static-only `generatedFrom`, and explicit `affectedProcessCount: null` because CodeLattice does not use the legacy GitNexus process model. New CLI regression tests and `scripts/codelattice-detect-changes-smoke.sh` cover git, non-git, and untracked-file paths.

- **MCP Tool Surface Consolidation**: adds 8 facade tools that orchestrate existing tools via a `mode` parameter, reducing the cognitive surface for AI agents. New facade tools: `codelattice_project` (overview/quality/insights/full), `codelattice_symbol` (search/context/callers/callees/graph), `codelattice_change_review` (changed_symbols/impact/production_assist/breaking_change/consistency/full_review), `codelattice_cleanup` (dead_code/reachability/external_api/framework_entries/safe_cleanup_review), `codelattice_workspace` (graph/impact/overview/full), `codelattice_release_check` (quick/full/config/docs_tests/breaking_changes), `codelattice_cache` (status/clear/explain), `codelattice_workflow` (onboarding/before_edit/after_edit/delete_code/release_check/legacy_cleanup). All lower-level tools remain fully available. Supports `CODELATTICE_MCP_TOOLSET=core` (facades + essentials) and `CODELATTICE_MCP_TOOLSET=full` (49 tools after current root-cause pack). All facade outputs include unified envelope with schemaVersion, tool, mode, summary, result, nextActions, cautions, generatedFrom (static-only/heuristic), and underlyingTools. Invalid mode returns structured error with validModes. compact=true strips result and keeps riskLevel summary. New facade handler functions with unwrap_tool_result, wrap_facade_output, validate_facade_mode, and safe_insert_tool helpers. Preflight doc: docs/plans/2026-05-19-mcp-tool-surface-consolidation.md.
...
- **WebUI Workspace Cross-Project Impact Pack**: adds workspace-level cross-project impact analysis that answers "if I change X, what else is affected?" New `GET/POST /api/workspace/impact` API performs BFS traversal on the workspace graph to find downstream/upstream affected nodes, computing risk levels (critical/high/medium/low/unknown) with human-readable reasons and review checklists. Target resolution supports 7 priority levels: exact nodeId (1.0), projectId (1.0), snapshotId (0.95), exact path (0.90), suffix path (0.75), label match (0.65), and fuzzy match (0.45). The insights API now includes `crossProjectImpactHints` with high-fanout projects, shared scripts/configs, and suggested impact targets. The UI adds an Impact Analysis section with summary cards, affected projects/assets tables, unsupported boundary warnings, risk reasons, review checklist, and a "Copy AI Summary" button. All output is static-only heuristic. Smoke test validates target resolution, BFS traversal, risk scoring, direction handling, error cases, and insights integration (21 checks).

- **WebUI Workspace Cross-Project Graph Pack**: adds workspace-level cross-project relationship graph intelligence. New `GET/POST /api/workspace/graph` API builds a `CodeLatticeWorkspaceGraphV1` graph from a workspace run, producing nodes (workspace, project, config, script, workflow, unsupported) and edges (contains, depends_on, imports, script_refs, config_refs, adjacent_to, unsupported_boundary). Static relationship extraction reads only manifest/config files (Cargo.toml, package.json, pyproject.toml, CMakeLists.txt, Makefile, cjpm.toml, CI workflows, Dockerfile, shell scripts) — no project code execution. Edges carry confidence (0.35–1.0), reason, and evidence. The insights API now includes `crossProjectGraphSummary` with node/edge counts, top connected projects, bridge scripts/configs, and unsupported boundaries. The UI adds a Cross-Project Graph section with summary cards, top connected projects, bridge items, a "Load Full Graph" button showing an edge table, and a "Copy Graph Summary for AI" button. All output is static-only heuristic. Smoke test validates schema, node/edge integrity, no dangling edges, insights integration, and error handling (15 checks).
...
- **WebUI Workspace Insight Polish Pack**: adds workspace-level health scoring, risk assessment, and curated recommendation lists to the Workspace view. New `/api/workspace/insights` API (GET+POST) computes per-project health scores (100→0 based on quality gates, automation risks, edge density, adjacent unsupported modules), generates Read First / Review First / Cleanup First recommendation lists, and extracts cross-project signals (unsupported language clusters, duplicated project labels). The UI renders an insight dashboard with health score, risk level, project score table, and recommendation lists. Report export supports Workspace Review Report title and includes workspace overview, project scores, and recommendation sections. All insights are static-only heuristic and clearly labeled as such.
...
- **MCP facade/workspace self-check hardening**: development wrapper and installer doctor now prefer the freshest full-tool binary instead of accepting a stale release binary; workspace graph/impact MCP outputs now include `schemaVersion` and static-only `generatedFrom` flags; dogfood now covers workspace tools, all 8 facade tools, and root-cause evidence assistant; local/source smoke thresholds were updated to the 49-tool profile.
...
- **WebUI Phase G — Live MCP Job Mode**: True MCP tool calls via runner, job lifecycle management, live result rendering in WebUI.
  ...
  - `scripts/webui-live-mcp-smoke.sh` — Smoke test: MCP status, tools list (38 tools), create project_overview job, poll until success, list jobs, error cases (bad workflow, missing root), cancel, delete (10+ checks).
...
- Packaging and smoke thresholds now expect the current 38-tool MCP profile for local `0.15.0-beta.1` candidate artifacts.
