...
- **MCP AI Toolset Default Pack**: changes the default MCP surface from full 49-tool exposure to an AI-friendly `ai` toolset. Default `tools/list` now returns facade-first entry tools (`codelattice_project`, `codelattice_symbol`, `codelattice_change_review`, `codelattice_cleanup`, `codelattice_workspace`, `codelattice_release_check`, `codelattice_cache`, `codelattice_workflow`) plus `codelattice_ai_context_pack` and `codelattice_root_cause_assistant`. `CODELATTICE_MCP_TOOLSET=core` exposes common low-level tools for advanced agents, and `CODELATTICE_MCP_TOOLSET=full` keeps all 49 tools for debugging and regression smoke. `initialize.serverInfo` now includes `toolset`, visible `toolCount`, `fullToolCount`, and `recommendedEntryTools`. Hidden tools return `tool_not_in_ai_toolset` / `tool_not_in_core_toolset` with facade and env-var guidance instead of letting agents guess.
