...
- **MCP AI Toolset Default Pack**: changes the default MCP surface from full 51-tool exposure to an AI-friendly `ai` toolset. Default `tools/list` now returns facade-first entry tools (`codelattice_project`, `codelattice_symbol`, `codelattice_change_review`, `codelattice_cleanup`, `codelattice_workspace`, `codelattice_release_check`, `codelattice_cache`, `codelattice_workflow`) plus `codelattice_ai_context_pack` and `codelattice_root_cause_assistant`. `CODELATTICE_MCP_TOOLSET=core` exposes common low-level tools for advanced agents, and `CODELATTICE_MCP_TOOLSET=full` keeps all 51 tools for debugging and regression smoke. `initialize.serverInfo` now includes `toolset`, visible `toolCount`, `fullToolCount`, and `recommendedEntryTools`. Hidden tools return `tool_not_in_ai_toolset` / `tool_not_in_core_toolset` with facade and env-var guidance instead of letting agents guess.
...
- **MCP facade/workspace self-check hardening**: development wrapper and installer doctor now prefer the freshest full-tool binary instead of accepting a stale release binary; workspace graph/impact MCP outputs now include `schemaVersion` and static-only `generatedFrom` flags; dogfood now covers workspace tools, all 8 facade tools, and root-cause evidence assistant; local/source smoke thresholds were updated to the 51-tool profile.
