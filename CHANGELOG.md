...
- **Risk calibration + issue triage pack**: project risk items now expose raw risk, rank-adjusted risk, calibrated priority bands, percentile bands, and tie-break explanations so AI agents do not treat equal-looking `high` lists as equally urgent. `codelattice_workflow(mode=ask)` now embeds a compact static `triagePlan` for issue-like questions when enough graph evidence exists, including likely areas, read-first files, hypotheses, impact hints, and explicit runtime/test/coverage evidence gaps. This remains dev-only for now; installed `CodeLattice-Tool` is not synced by this change.
