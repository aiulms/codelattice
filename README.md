...
CodeLattice 是一个面向 **AI 编程工作流** 的本地代码图谱引擎。它通过静态分析把大型代码库里的项目结构、符号、调用关系、依赖边界、质量信号和变更影响面整理成可查询、可复跑、可审计的上下文，让 AI 助手和开发者在修改代码前先理解系统。

它适合接手陌生大仓、维护 legacy codebase、评估重构风险、做提交前审查，以及给 Codex、Claude Desktop、opencode 等本地 AI 工具提供 MCP sidecar。CodeLattice 默认只读扫描源码，不上传代码，不执行目标项目构建脚本；结果可以通过 CLI、JSON 输出和 MCP 工具集消费。
