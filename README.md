...
CodeLattice 是一个 **本地代码智能引擎**：面向大型、遗留、复杂代码库做静态图谱分析，把源码中的项目结构、符号、调用关系、依赖边界和质量信号转成 AI 编程助手与工程工具都能稳定消费的本地上下文。
...
CodeLattice 用 Rust 编写，当前 beta 支持 Rust、Cangjie / 仓颉、ArkTS、TypeScript、C、C++、Python 七条本地图谱分析路径，并提供 CLI 与 MCP sidecar 两种使用方式。

**当前状态：外部 Beta / daily-use candidate（`v0.14.0-beta.1`）**。本地生产试用与 release smoke 已通过，但还不是 GA。CLI 输出、MCP contract 和质量门在 beta 阶段仍可能以兼容优先的方式演进。完整变更见 [CHANGELOG](CHANGELOG.md)，验证矩阵见 [Smoke Matrix](docs/release/smoke-matrix.md)。
...
- **AI 代码审查上下文**：提供 symbol context、calls_from / calls_to、impact_preview、changed_symbols、review_plan、production_assist 等 MCP 工具。
...
| 通用多语言扫描器 | 对 Rust 与 Cangjie 有更深的项目模型、fixture、质量 gate 和调用策略；对 ArkTS / TypeScript / C / C++ / Python 提供 beta 静态图谱 |
...
默认 release build 会启用 Cangjie / ArkTS / TypeScript / C / C++ / Python 全语言 feature。
...
**外部 Beta / daily-use candidate（`v0.14.0-beta.1`）**：本地生产试用与 release smoke 已通过，但不是 GA。
