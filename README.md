...
它用 Rust 编写，目前重点支持 Rust 与 Cangjie / 仓颉项目，并逐步覆盖 ArkTS / HarmonyOS、TypeScript、C/C++ 与 Python。整个过程默认只读，不上传代码，不依赖云端索引。
...
- **先看清再下手**：通过 project overview、symbol search、call graph 和质量 gate，先定位大文件、高扇出符号、低置信度调用和潜在风险区域。`project_insights` 工具进一步提供大项目洞察地图：入口点候选、热点文件/符号、风险区域、低置信度聚集区和先读/先审建议。
...
- **大项目洞察**：`project_insights` 基于 graph 启发式识别入口点、热点文件/符号、风险区域和低置信度聚集区，提供"先读哪里"和"改前重点审查哪里"的建议，适合接手陌生大仓的第一步。
...
| 通用多语言扫描器 | 对 Rust 与 Cangjie 有更深的项目模型、fixture、质量 gate 和调用关系策略；对 C/C++/Python 提供 Phase A 静态图谱 |
...
| Python | **Phase A** | `tree-sitter-python` |
