...
**当前状态：外部 Beta（`v0.13.0-beta.1`）**。本地生产试用已通过，但还不是 GA 版本。完整变更见 [CHANGELOG](CHANGELOG.md)，验证矩阵见 [Smoke Matrix](docs/release/smoke-matrix.md)。
...
# 构建默认 Rust + Cangjie 支持的 release binary
...
当前 `v0.13.0-beta.1` 已发布 macOS Apple Silicon（`darwin-arm64`）包：
...
**外部 Beta（`v0.13.0-beta.1`）**：本地生产试用已通过，但不是 GA。
