...
| Cangjie stdout purity | `crates/cangjie/vendor/tree-sitter-cangjie/src/scanner.c` | fprintf→stderr |
| Debug output stdout 泄露 | `crates/cli/src/main.rs` | `#[cfg(debug_assertions)]` guard |
...
1. **tree-sitter-cangjie scanner.c**：`logger()` 函数使用 `fprintf(stdout, msg)` 输出调试信息（即使 `DEBUG_SCANNER=0` 时不触发，但编译器会产生 unused parameter warning）
2. **CLI main.rs**：`eprintln!("分析中...")` 在所有 build profile 下执行

### 修复

- scanner.c：`fprintf(stdout, msg)` → `fprintf(stderr, "%s", msg)`，修复格式化参数安全性
- main.rs：在 `eprintln!` 外加 `#[cfg(debug_assertions)]`，release build 不输出
...
`generatedAt` 只保证字段存在和 ISO 8601 格式稳定，不参与 strict deterministic 比较。deterministic 测试已排除此字段。
