...
**Commits:** `02cab82`, `025ce01`, plus follow-up review fix
...
|---------|----------|-------------|---------|-------|-------|-------------|----------|---------------|
| lodash-es | javascript | 48 | 10,775 | 14,471 | 3,592 | 3,181 | 1 | 2 |
| uuid | javascript | 39 | 86 | 163 | 21 | 46 | 0 | 3 |
| zod | typescript | 401 | 2,845 | 47,358 | 34,636 | 550 | 0 | 0 |
| tinybench | typescript | 72 | 97 | 3,783 | 3,101 | 62 | 0 | 0 |
| vite-plugin-react | typescript | 396 | 849 | 9,158 | 6,534 | 648 | 0 | 0 |
| conf | typescript | 7 | 68 | 2,214 | 1,612 | 46 | 0 | 0 |

All 6 real-world projects cloned from GitCode mirrors (`gh_mirrors/{prefix}/{name}`). Corpus projects are analyzed with their manifest-pinned language instead of re-detecting from root-level files; this avoids false baselines for projects whose root `package.json` / `tsconfig.json` shape does not match the intended validation lane.
...
4. **Manifest language drift**: Corpus entries were added as paths only, then re-detected by local file markers. This misclassified `uuid` and `vite-plugin-react` relative to the manifest. Fixed by storing path + language + project id per entry, and by including skipped corpus entries in the JSON report.
...
- `scripts/js-ts-corpus-smoke.sh --corpus-dir ... --offline`: 8/8 pass, 0 skipped

## Files Changed

| File | Action | Lines |
|------|--------|-------|
| `scripts/js-ts-corpus-smoke.sh` | Rewritten | ~330 |
| `fixtures/corpus/js-ts-corpus-manifest.json` | New | ~60 |
| `CHANGELOG.md` | Updated | +8 lines |

## Deferred Work

- **Real-project baselines**: Rerun `--clone-missing --corpus-dir /path/to/corpus` when refreshing the GitCode mirror cache.
