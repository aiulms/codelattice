...
CodeLattice 当前已经具备多语言静态图谱分析、workspace 发现、跨项目图谱、MCP facade 和 WebUI workbench。MCP 并发稳定性已经通过 `mcp_server_busy` 守住了当前可用性：AI 客户端并行调用不会再打断 MCP session。

下一阶段的核心不是继续修 MCP stdio，也不是按外部发行节奏拆成多个产品版本。CodeLattice 是我们自己首先使用的分析内核，目标直接定义为 **Analysis Engine 1.3**：

> 面向大项目和大 workspace，把 discovery / parse / symbol / import / reference / calls / graph merge 拆成可并行、可缓存、可增量、可解释的分析任务，并由统一 scheduler 管理。

这份路线图用于指导后续执行 AI 分包实现。文档里的 Pack 是实施切片，不是版本路线；所有切片都服务于同一个 1.3 目标：大项目真并发分析内核。
...
## Target: Analysis Engine 1.3

目标：大项目 / 大 workspace 多项目并发、增量 cache、共享中间产物、paged graph result 和 MCP job runtime 成熟可用。
...
- job id / status / dedupe / worker limit
- cancel / resume
- compact result / paged detail
...
- 不把 MCP busy guard 当成终点。
- 不把单项目 file-level 并发当成终点。
- 不为外部发行节奏牺牲内部大项目分析体验。

实施仍然可以分包，但每个包都必须声明它如何推进 1.3，而不是只完成局部“可用版”。
...
When handing this roadmap to an execution AI, frame the work as **Analysis Engine 1.3** from the start. Use implementation slices only to keep risk bounded; every slice must explicitly serve the same 1.3 target.

1. “Implement Analysis Engine 1.3 Foundation Pack: Analysis DAG Contract + Deterministic Graph Merge. Preserve behavior and prove shuffled artifacts produce stable output.”
2. “Implement Analysis Engine 1.3 Parallel File Analysis Pack for Rust + TypeScript/JavaScript + Python, with serial-vs-parallel parity.”
3. “Implement Analysis Engine 1.3 Shared Intermediate Cache Pack for parse/symbol/import/reference artifacts.”
4. “Implement Analysis Engine 1.3 Language Adapter Concurrency Contract rollout for remaining adapters.”
5. “Implement Analysis Engine 1.3 MCP Job Runtime backed by the parallel analysis engine.”
6. “Implement Analysis Engine 1.3 Large Workspace Benchmark Pack using real corpora and open-nwe-class workspace validation.”
...
- All supported languages declare engine capabilities; Rust, TypeScript, JavaScript, and Python must be fully engine-backed in the first usable 1.3 slice.
...
Start with **Analysis Engine 1.3 Foundation Pack: Analysis DAG Contract + Deterministic Graph Merge**. This is the required substrate for 1.3. The implementation prompt should explicitly say the final target is Analysis Engine 1.3 large-project true concurrency, and this pack is the foundation slice.
