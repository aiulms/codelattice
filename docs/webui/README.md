...
- WebUI 消费的是聚合后的只读 snapshot，不是直接调用 37 个 MCP 工具
- MCP 仍是 AI 主通道；WebUI 是给人类看的本地项目理解界面
- 本轮不做完整 WebUI，只做 readiness 和 snapshot contract
...
| 3 | Impact | 改这个 symbol 会影响什么？ | `impact_preview`, `impact_analysis`, `changed_symbols` |
