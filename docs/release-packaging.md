...
The current master packaging target is `v0.17.0-beta.2` with a `darwin-arm64` beta tarball and checksum. Multi-platform artifacts are planned next.
...
```bash
bash scripts/package-release.sh --version 0.17.0-beta.2
...
codelattice-<version>-<platform>/
  ...
  fixtures/
    ...
    javascript/portable-smoke/
    c/portable-smoke/
    cpp/portable-smoke/
    python/portable-smoke/
    shell/portable-smoke/
```
...
- build features
- MCP profile: server version, Cangjie/ArkTS/TypeScript/JavaScript/C/C++/Python/Shell support, tool count
...
- wrapper `--self-test`
- MCP `tools/list >= 49` in `CODELATTICE_MCP_TOOLSET=full`
...
- JavaScript portable fixture analyze
- C portable fixture analyze
- C++ portable fixture analyze
- Python portable fixture analyze
- Shell portable fixture analyze
...
bash scripts/package-release.sh
bash scripts/release-smoke.sh
bash scripts/install-release.sh --dry-run --version v0.17.0-beta.2 --platform darwin-arm64
