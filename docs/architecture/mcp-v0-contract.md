...
`codelattice_workflow` 是 AI-friendly toolset 的推荐第一入口。它不直接执行项目分析，而是把用户意图路由成可调用的下一步动作，避免 AI 在 50 个底层工具之间猜测。默认 `ai` toolset 只暴露 6 个入口工具；底层工具通过 `core` / `full` 显式开启。
