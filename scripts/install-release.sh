...
Options:
  --version <tag>       Release tag or version (default: v0.1.0)
...
download_file() {
    ...
    if ! curl -fL --retry 3 --connect-timeout 20 -o "$out" "$url"; then
        ...
        echo "This release may not provide platform '$PLATFORM'. Current v0.1.0 ships darwin-arm64; other platforms can clone and build from source." >&2
        exit 1
    fi
}
