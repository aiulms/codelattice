...
DEFAULT_VERSION="v0.13.0-beta.2"
...
Options:
  --version <tag>       Release tag or version (default: v0.13.0-beta.2)
...
download_file() {
    ...
    if ! curl -fL --retry 3 --connect-timeout 20 -o "$out" "$url"; then
        ...
        echo "This release may not provide platform '$PLATFORM'. Current v0.13.0-beta.2 ships darwin-arm64; other platforms can clone and build from source." >&2
        exit 1
    fi
}
