...
CODELATTICE_MCP_BIN="$BINARY" CODELATTICE_MCP_REQ="$REQ" CODELATTICE_MCP_OUT="$OUT" CODELATTICE_MCP_ERR="$ERR" python3 - <<'PY'
import os
import subprocess
import sys
...
with open(os.environ["CODELATTICE_MCP_REQ"], "r", encoding="utf-8") as stdin, \
    ...
    try:
        proc = subprocess.run(
            [os.environ["CODELATTICE_MCP_BIN"], "mcp"],
            stdin=stdin,
            stdout=stdout,
            stderr=stderr,
            env=env,
...
CODELATTICE_MCP_BIN="$BINARY" CODELATTICE_REPO_ROOT="$REPO_ROOT" python3 - <<'PY'
import json
import os
import subprocess
import sys
...
binary = os.environ["CODELATTICE_MCP_BIN"]
