...
curl -s -X POST "$B/api/fs/pick-directory" -H "Content-Type: application/json" -d '{"dryRun":true}'|python3 -c "import json,sys;d=json.load(sys.stdin);assert d['success'] and 'supported' in d['data']" 2>/dev/null && pass "folder picker dry-run"||fail "folder picker dry-run"
