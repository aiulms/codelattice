...
if [[ -n "$FEATURES" ]]; then
    ...
    echo ""
    echo "  # Cangjie Bridge 格式"
    echo "  $BIN_PATH analyze --root fixtures/cangjie/portable-smoke --language cangjie --format gitnexus-rc --strict"
    echo ""
fi
echo "  # 自身分析"
...
echo ""
echo "  # Bridge 格式验证（端点完整性 + 结构）"
echo "  ./scripts/verify-bridge.sh"
echo ""
