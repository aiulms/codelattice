...
for doc in \
    ...
    docs/architecture/mcp-v0-contract.md; do
...
for script in scripts/linux-source-build-smoke.sh; do
    if [[ -f "$REPO_ROOT/$script" ]]; then
        mkdir -p "$STAGE_DIR/$(dirname "$script")"
        cp "$REPO_ROOT/$script" "$STAGE_DIR/$script"
...
assert int(s.get("toolCount", 0)) >= 37
...
cat > "$STAGE_DIR/manifest.json" <<JSON
{
  ...
  "buildFeatures": "$RELEASE_FEATURES",
